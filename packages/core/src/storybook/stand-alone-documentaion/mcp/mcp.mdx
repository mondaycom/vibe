...
- [`dropdown-migration`](#dropdown-migration) - Tool for migrating from old Dropdown to new Dropdown (Alpha)
...
Specialized migration assistant for upgrading from the old Dropdown component to the new Dropdown available in `@vibe/core/next`.
...
- Better TypeScript support with full generics
- Built-in form field integration (label, helperText, error, required)
- Improved performance without react-select dependency
- Enhanced tooltip support with `tooltipProps`
