...
- [Breaking cCanges 🚨](#breaking-changes-)
  - [General](#general)
...
```tsx
import { Button, ButtonProps, ButtonSize } from "monday-ui-react-core";
```

### Improved prop type-checking

Now you can use type-safe strings instead of static enums for props, providing a better and faster developement experience:
...
When usedd inside Modals, the floating components (i.e., Tooltip, Tipseen, Dialog) will now be rendered inside the Modal's container by default, ensuring they are always on top. This change prevents any z-index interference and ensures a consistent user experience.

{/* <img href={layers.gif} alt="floating elements on modal"></img> */}
...
1. Install the `@vibe/core` (and `@vibe/icons`, if needed) and remove `monday-ui-react-core`:
