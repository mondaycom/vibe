...
import { migrationTable } from "./typography/assets";
...
Vibe 3 is a major update to the Vibe design system, introducing several new features, enhancements, and breaking changes. This guide will walk you through the these changes to help you transition smoothly from v2 to v3.
...
Please note that most of the changes are covered by the migration script, but some manual changes may still be required as some change require manual intervention (whether regarding style, UX, or behavior).
...
<Tip title="Be aware of uncovered changes" emoji="🚩">
  Changes that are not covered by the automatic migration are marked with a 🚩 in the [Breaking
...
Now you can use type-safed string instead of enums (exported as static properties) for props, giving a better developer experience:
...
For to replace old typography usages follow the migration table below:
...
These components were **written from scracth** with new API and style, with a better accessiblity and usability:
...
- The "Spin In" animation removed, and so the `isAnimationDisabled` prop has been removed
...
While we recommend migrating to Vibe 3 to take advantage of the new features and improvements, you can continue using Vibe 2. However, please note that Vibe 2 will no longer be recieve any new features and will reach EOL upon the next major version (Vibe 4) release.
