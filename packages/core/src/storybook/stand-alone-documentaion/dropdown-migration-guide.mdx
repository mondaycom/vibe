...
<Dropdown
  onChange={(option, meta) => {
    console.log(option.value, meta.action);
  }}
/>
...
### 4. Update Multi-Select Configuration
...
<Dropdown
  ...
  value={selectedOptions}
  onChange={options => setSelectedOptions(options)}
  multiline // Optional: display selections in multiple lines
/>
`} />
...
<Dropdown label="Select an option" helperText="Helper text here" required error={hasError} options={options} />
`} />

### 6. Update Custom Filtering
...
<Dropdown
  ...
  showSelectedOptions={false} // Hide selected options from list
/>
`} />
...
- **`isOptionSelected`** - Handled automatically
- **`tabSelectsValue`** - Handled automatically
- **`blurInputOnSelect`** - Handled automatically
- **`inputValue`** - Use controlled `value` instead
...
- **43% smaller bundle size** compared to react-select implementation
- **Faster rendering** with native implementation
- **Better memory usage** with optimized re-renders
- **Improved accessibility** with proper ARIA implementation
...
<Dropdown placeholder="Select an option" options={options} onChange={option => setSelected(option)} />
`} />
...
<Dropdown searchable placeholder="Search options..." options={options} onInputChange={value => setSearchTerm(value)} />
`} />

### Pattern 3: Multi-Select with Form
...
### Pattern 4: Custom Option Rendering
...
// After (same structure, but with enhanced features)

<Dropdown
  options={groupedOptions}
  withGroupDivider
  stickyGroupTitle // New feature!
/>
`} />
...
A: Yes, the old Dropdown will be deprecated once the new one moves out of Alpha. We'll provide ample notice and migration support.
...
**Q: What about virtualization for large datasets?**
A: The new Dropdown has built-in performance optimizations. For extremely large datasets (1000+ items), consider implementing server-side filtering.
...
- **Migration Support**: Check our [main migration guide](/?path=/docs/migration-guide--docs) for general Vibe 3 migration help
