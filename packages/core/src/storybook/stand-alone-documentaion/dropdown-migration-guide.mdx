...
- **`showSelectedOptions`** - **BREAKING CHANGE**: Defaults to `false`. In the old Dropdown, selected options were always visible in single select mode.
...
- **`showSelectedOptions`** - Control selected options visibility (defaults to `false`)
...
**Q: Why are my selected options hidden from the dropdown list?**
A: The new Dropdown defaults to `showSelectedOptions={false}` to improve UX. Set `showSelectedOptions={true}` to restore the old behavior.
