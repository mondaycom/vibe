...
<UsageGuidelines
  guidelines={[
    ...
    <>
      AccordionItem automatically receives <code>aria-expanded</code> and <code>aria-controls</code> attributes. The
      button is automatically linked to its content region using these attributes.
    </>,
    <>
      When expanded, AccordionItem content receives <code>role="region"</code> to identify it as a landmark for screen
      readers.
    </>,
    <>
    ...
    <>
      Use <code>allowMultiple</code> prop appropriately. Set to <code>true</code> when users may need to reference
      multiple sections simultaneously, or <code>false</code> when sections are mutually exclusive.
    </>,
    <>
