...
import { ColorPickerValueOnly, ColorShapes } from "../../ColorPickerConstants";
...
const ColorPickerItemComponent: VibeComponent<ColorPickerItemComponentProps> = forwardRef(
  (
    ...
    const disabledColor = isMondayColor ? ColorUtils.getMondayColorAsStyle(color, ColorStyle.SELECTED) : color;
    ...
    useEffect(() => {
      if (!itemRef?.current || shouldRenderIndicatorWithoutBackground || !isMondayColor || isDisabled) return;
    ...
    }, [color, colorAsStyle, colorStyle, isMondayColor, itemRef, shouldRenderIndicatorWithoutBackground, isDisabled]);
    ...
    const shouldRenderIcon =
      !isDisabled && ((isSelected && !isEditing) || ColorIndicatorIcon || (isEditing && isHovered));
      ...
      <Tooltip content={tooltipContent}>
        <li
          className={cx(styles.itemWrapper, {
            [styles.selectedColor]: isSelected && !isEditing,
        ...
        >
          ...
          >
            <div className={cx(styles.colorIndicatorWrapper)} style={colorIndicatorWrapperStyle}>
              {shouldRenderIcon && (
                <Icon
                  icon={isEditing ? Edit : isSelected ? SelectedIndicatorIcon : ColorIndicatorIcon}
                  className={cx({
