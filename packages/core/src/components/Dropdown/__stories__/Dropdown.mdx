...
<UsageGuidelines
  guidelines={[
    ...
    "Use `inputAriaLabel` prop when you need to provide a specific accessible name for the input field.",
    "Use `menuAriaLabel` prop when you need to provide a custom accessible name for the dropdown menu.",
    "Use `menuId` prop to provide a specific ID for the menu container for screen reader navigation.",
    "Use `tabIndex` prop for custom keyboard navigation order when needed.",
    "Use `disabled` prop appropriately to indicate when dropdowns are not available for interaction.",
    "Use `readOnly` prop when users should see but not modify the dropdown value."
  ]}
/>
