...
export class MetadataService {
  ...
  static async getMetadata(): Promise<ComponentMetadata[]> {
    if (this.cachedMetadata) {
      console.error("[EZDS MCP] Using cached metadata");
      return this.cachedMetadata;
    }

    try {
      console.error("[EZDS MCP] Fetching metadata from unpkg...");
      const metadata = await this.fetchWithRetry();
      console.error(`[EZDS MCP] Fetched ${metadata.length} components, caching for session`);
      this.cachedMetadata = metadata;
      return metadata;
    } catch (error) {
      console.error("[EZDS MCP] Failed to fetch metadata:", error);
