...
const configWithUseRecommendation = {
  ...config,
  rules: {
    "@ezds/web/use-defined-css-var-when-available": [
      true,
      {
...
describe("@ezds/web/use-defined-css-var-when-available", () => {
...
--spacing-medium
 (@ezds/web/use-defined-css-var-when-available)`
    ...
    expect(secondWarning.text).toBe(
      `Expected \"16px\" to be \"var(--border-radius-big)\" (@ezds/web/use-defined-css-var-when-available)`
    );
    ...
    it(`does not lint if the rule is disabled by config value of "${ruleConfigValue}"`, async () => {
      const { results } = await lint({
        ...
        config: {
          ...config,
          rules: {
            "@ezds/web/use-defined-css-var-when-available": ruleConfigValue
          }
        }
      });
