...
<UsageGuidelines
  guidelines={[
    ...
    <>
      When the banner text should be announced by screen readers (e.g., for dynamic updates), set the `ariaLive` prop on
      `AlertBannerText` to control the live region behavior ("polite", "assertive", or "off"). By default,
      `AlertBannerText` does not set a live region.
    </>
  ]}
/>

<Canvas of={AlertBannerStories.WithAriaLive} />
