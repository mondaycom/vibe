...
<Tip title="Migration Guide Available" emoji="🚀">
  ...
  <Link href="/?path=/docs/components-list-new-migration-guide--docs">List Migration Guide</Link> for step-by-step
  instructions, breaking changes, and new features.
</Tip>
...
<UsageGuidelines
  guidelines={[
    ...
    <>
      When the <code>disabled</code> prop is set on the List, all keyboard navigation is blocked.
    </>,
    <>
...
<Tip>
  Note that keyboard navigation is handled by the virtualization library when using this pattern. The List's built-in
  keyboard navigation works best with non-virtualized lists.
</Tip>
