...
<Tip>
  Note that keyboard navigation is handled by the virtualization library when using this pattern. The List's built-in keyboard navigation works best with non-virtualized lists.
</Tip>
