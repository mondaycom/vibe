...
Set `removable: false` on an option to prevent its chip from being removed in multi select mode. The chip renders without a remove button and is protected from keyboard deletion and clear-all.

<Canvas of={DropdownBasicDropdownStories.NonRemovableOptions} />
