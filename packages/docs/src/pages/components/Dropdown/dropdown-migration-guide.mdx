...
- **`withMandatoryDefaultOptions`** - Removed. Mark individual options with `removable: false` instead (see [Non-Removable Options](#non-removable-options-))
...
In the old Dropdown, keeping specific chips from being removed in multi-select mode required the dropdown-level `withMandatoryDefaultOptions` prop together with an option-level `isMandatory: true` flag. The new Dropdown replaces both with a single, self-describing option-level `removable` flag. Set `removable: false` on an option to render its chip read-only (no remove button) and protect it from keyboard deletion and clear-all.
