...
- The `@ezds/web/src/Icons` **was removed**, use `@vibe/icons/raw` instead to use svg icons:

<ul>
  <DiffCodeBlock
    code={`- import Close from "@ezds/web/src/Icons/Close.svg";
...
- The `type` prop was removed. The `Search.types.SQUARE`, `ROUND`, and `UNDERLINE` variants are no longer available
...
<ul>
  <DiffCodeBlock
    code={`- <Search size={Search.sizes.SMALL} />
+ <Search size="small" />
`}
  />
</ul>
