...
- **`monday-ui-style` renamed to `@vibe/style`** — Only relevant if you use this internal styling package directly. Update all import paths and stylelint config references:
...
The legacy DatePicker (based on `react-dates` and `moment.js`) has been replaced with a new implementation with a different visual design and API, using native `Date` objects and `date-fns`.
...
The `"stretch"` value has been removed from the `justify` prop:
...
The legacy Modal has been removed and replaced with a completely new Modal component with a different API. The new Modal was previously available via `@vibe/core/next`.
...
<ul>
  <DiffCodeBlock
    code={`- import { Modal, ModalContent, ModalFooterButtons } from "@vibe/core";
...
The deprecated `getItemHeight` and `onVerticalScrollbarVisiblityChange` props have been removed. Use `itemHeight` and `onScrollbarVisibilityChange` instead.
...
`padding-inline-start` reduced from `16px` to `8px` across TextField, BaseInput, TextArea, and Dropdown Trigger. No code changes needed, but check custom CSS overrides.
