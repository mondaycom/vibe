...
- The dropdown-level `withMandatoryDefaultOptions` prop and the option-level `isMandatory: true` flag are replaced by a single option-level `removable: false` flag to keep a chip from being removed in multi-select mode
