...
// External functions
export function getWithin(canvasOrValidTestElement: HTMLElement | QueriedHTMLElement) {
...
export const pressNavigationKey = async (command = NAVIGATIONS_COMMANDS.TAB, waitForDebounceMs = 0) => {
  const promise = command === NAVIGATIONS_COMMANDS.TAB ? userEvent.tab() : userEvent.keyboard(command);
  const result = await promise;
  await delay(waitForDebounceMs);
  return result;
};
...
export const keyboardMultipleTimes = async (text: string, count: number, options = { delay: 70 }) => {
