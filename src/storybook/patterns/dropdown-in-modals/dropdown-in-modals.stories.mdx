...
import styles from "./dropdown-in-modals.module.scss";
...
Our dropdowns are implemented using the React Select library.
...
To handle straightforward scenarios of displaying dropdowns inside a container with relevant overflow styles, you can utilize our dropdown's 'insideOverflowContainer' prop.
...
The 'insideOverflowContainer' prop will not function as expected for dropdowns displayed inside Vibe's dialog components.
...
For the most complex use cases, you can utilize our dropdown's "popupsContainerSelector" prop.
...
If you have defined the dropdown's popovers container styles as required, you can use it as demonstrated in the following example <Link href="/?path=/story/technical-patterns-display-dropdowns-inside-pop-overs--popups-container-selector" withoutSpacing>(click here for a live demo)</Link>

<Canvas className={styles.hideStory}>
  <Story name="popupsContainerSelector">
    {() => {
      const [show, setShow] = useState(false);
      const closeModal = useCallback(() => {
        setShow(false);
