...
import { EasingDo, EasingDont, ConsistencyDo, ConsistencyDont, SimplicityDo, SimplicityDont } from "./assets";
...
<ComponentRules
  rules={[
    {
      positive: {
        component: [<video height="180px" src={SimplicityDo} controls loop />],
        description: "Use only one primary action within a single view."
      },
      negative: {
        component: [<video height="180px" src={SimplicityDont} controls loop />],
...
When scaling, components should not be streched, keep the X and Y proporsions locked
