...
```javascript
import { Heading } from "monday-ui-react\next";
```

In the upcoming major release of Vie packages (3.0), we will remove LegacyHeading from the main entry, and then you will be able to import this Heading component
...
<Tip title="Not what you were looking for?">
  ...
  <Link href="/?path=/docs/inputs-editableheading--overview" withoutSpacing size={Link.sizes.SMALL}>
    EditableHeading
  </Link>{" "}
    ...
    >
      ...
      <div>
        <Heading type={Heading.types.H2} ellipsis withoutTooltip maxLines={2}>
          Heading with two lines overflow heading with 2 lines overflow heading with two lines overflow heading with 2
          lines overflow heading with two lines overflow heading with two lines overflow heading with two lines overflow
        </Heading>
      </div>
    </Flex>
  </Story>
