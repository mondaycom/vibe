...
<Canvas>
  <Story name="Overview" args={{ariaLabel: "ariaLabel"}}>
...
<UsageGuidelines guidelines={[
  <>Icon button will always appear with a <Link href="/?path=/docs/components-tooltip--overview">tooltip</Link> while hovering, defining the icon’s meaning.</>,
  ...
  rules={[
    {
      ...
      negative: {
        ...
        description: <>Don’t use Icon button as the main action on the page. Instead, use the <Link href="/?path=/docs/components-button--overview" withoutSpacing>Button component</Link> with an icon.</>
      }
    },
    {
      positive: {
