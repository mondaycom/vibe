...
export const DatePicker = props => {
  ...
  return (
    <DialogContentContainer className={"datepicker-dialog-content-container"}>
      {props.range ? (
        <DatePickerComponent data-testid="date-picker"  date={date.startDate} endDate={date.endDate} onPickDate={d => setDate(d)} {...props} />
      ) : (
        <DatePickerComponent data-testid="date-picker"  date={date} onPickDate={d => setDate(d)} {...props} />
      )}
