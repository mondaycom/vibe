...
<Tip>When there’s limited space or no default selection, consider using a select <Link href="/?path=/docs/components-dropdown--overview" size={Link.sizes.SMALL} withoutSpacing>Dropdown</Link> instead.</Tip>
  ...
  rules={[
    {
      ...
      negative: {
        ...
        description:<>Don't use radio buttons when allowing the user to select more than one item from a list. In this case, use <Link href="/?path=/docs/components-checkbox--overview" withoutSpacing>checkboxes</Link> instead.</>
      }               
    },
    {
      positive: {
