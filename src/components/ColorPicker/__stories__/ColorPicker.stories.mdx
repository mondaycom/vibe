...
import {
  ...
  colorPickerWithCustomPicker,
  colorPickerWithCustomPickerAndMultiSelect
...
Passing `showCustomColorPicker` adds a custom color picker icon

<Canvas>
  <Story name="Custom Single">{colorPickerWithCustomPicker.bind({})}</Story>
</Canvas>

Also supports isMultiselect

<Canvas>
  <Story name="Custom Multi">{colorPickerWithCustomPickerAndMultiSelect.bind({})}</Story>
</Canvas>
