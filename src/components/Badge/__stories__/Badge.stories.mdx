...
When using Badge on a Button element, use alignment of CORNER in order to attach it to the element

<Canvas>
  <Story name="Button">
    <Badge alignment={Badge.alignments.CORNER}>
...
When using Badge on an Avatar element, use alignment of EMBEDDED in order to attach it to the element

<Canvas>
  <Story name="Avatar">
    <Badge alignment={Badge.alignments.EMBEDDED}>
...
<ComponentRules
  rules={[
    ...
    },
    {
      ...
      negative: {
        component: (
          <Badge alignment={Badge.alignments.EMBEDDED} anchor={Badge.anchors.TOP_START}>
            <Avatar size={Avatar.sizes.MEDIUM} src={person} type={Avatar.types.IMG} />
          </Badge>
        ),
