...
const TEST_ID = "test-text-area-field";
...
describe("TextAreaField tests", () => {
    ...
    beforeEach(() => {
        ...
        inputComponent = render(
            <TextAreaField placeholder={defaultPlaceHolder}
    ...
    it("should not change the value instantly when debounce value is provided", () => {
        ...
        inputComponent = rerender(
            <TextAreaField placeholder={defaultPlaceHolder} onChange={onChangeStub} id={TEST_ID} debounceRate={200}/>
        );
        const value = "Value of input";
        const input = screen.getByPlaceholderText(defaultPlaceHolder);
        fireEvent.change(input, {target: {value}});
    ...
    it("should be able to forward ref", () => {
        ...
        expect(ref.current.className).toMatch("textAreaField");
    });
    ...
    it("should call the debounced function after time passed (fake timers)", async () => {
        ...
        inputComponent = rerender(
            <TextAreaField placeholder={defaultPlaceHolder} onChange={onChangeStub} id={TEST_ID}
    ...
    it("char count should display correctly after changing value", () => {
        ...
        let value = "hello";
        rerender(<TextAreaField placeholder={defaultPlaceHolder} onChange={onChangeStub} showCharCount value={value}/>);
        const input = screen.getByPlaceholderText(defaultPlaceHolder);
    ...
    describe("autocomplete", () => {
        it("should add autocomplete attr and set it to on", () => {
            const {container} = render(
                <TextAreaField placeholder={defaultPlaceHolder} onChange={onChangeStub} id={TEST_ID} autoComplete="on"/>
            );
    ...
    it("should trim the value if trim is true", () => {
        const {rerender} = inputComponent;
        inputComponent = rerender(<TextAreaField placeholder={defaultPlaceHolder} onChange={onChangeStub} id={TEST_ID}
                                                 trim/>);
        const input = screen.getByPlaceholderText(defaultPlaceHolder);
        const value = "Value of input      ";
        fireEvent.change(input, {target: {value}});
    ...
    describe("a11y", () => {
        it("should add the aria label", () => {
            ...
            const {getByLabelText} = render(<TextAreaField inputAriaLabel={ariaLabel}/>);
