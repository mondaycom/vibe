...
<Canvas>
  <Story name="Dropdown with lables">
    {() => {
      const options = useMemo(() => [
        { value: "Success", label: "Success",  text: "Success", color: Label.colors.POSITIVE },
        { value: "Failed", label: "Failed", text: "Failed", color: Label.colors.NEGATIVE},
        { value: "In progress", label: "In progress", text: "In progress"}
      ], []);
      return (
        <Dropdown
          placeholder="Placeholder text here" 
