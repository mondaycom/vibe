...
<Canvas>
  <Story name="Overview" args={{ placeholder: "Placeholder text here", className: "dropdown-stories-styles_spacing" }}>
...
<Tip>
  If you don’t want to keep the user’s choice visible, use the{" "}
  <Link href="/?path=/docs/inputs-menu--overview" size={Link.sizes.SMALL} withoutSpacing>
    Menu component
  </Link>{" "}
  instead
</Tip>
  ...
  rules={[
    {
      ...
      negative: {
        ...
        description: (
          <>
            Don’t insert any fields inside a dropdown. If you need, use the{" "}
    ...
    {() => {
      ...
      return (
        <div>
          ...
          />
          <span> You can also keep it opened when selecting:</span>
          <Dropdown
            defaultValue={[options[0]]}
            options={options}
            multi
