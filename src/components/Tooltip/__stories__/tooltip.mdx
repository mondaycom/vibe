      ...
      positive: {
        component: (
          <Tooltip shouldShowOnMount content="Open quick search  + B">
            <IconButton icon={Bolt} iconSize="32" kind={IconButton.kinds.SECONDARY} />
          </Tooltip>
        ),
...
Immediately show when another one is shown. The two left tooltips use the `immediateShowDelay` show prop with a `0` value, and the right one doesn't, therefore it has a show delay regardelss of the other two.
