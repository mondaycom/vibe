...
<Tip title="Check yourself">If you are taking users through a multistep process, use the <Link href="/?path=/docs/navigation-multistepindicator--overview" size={Link.sizes.SMALL} withoutSpacing>Wizard</Link> component instead.</Tip>
...
### Indicative 
Use when there’s a need for a context and item’s location.
<Canvas>
  <Story name="Indicative ">
     <div className="monday-storybook-breadcrumbs-bar_column-wrapper">
        <div className="monday-storybook-breadcrumbs-bar_line" >
          <Icon icon={Board} />
          <span className="monday-storybook-breadcrumbs-bar_title">Weekly to-do</span>
        </div>
        <BreadcrumbsBar type={BreadcrumbsBar.types.NAVIGATION}>
          <BreadcrumbItem text="Rotem Dekel" />
          <BreadcrumbItem text="Marketing team planing" />
        </BreadcrumbsBar>
      </div>
  </Story>
</Canvas>
