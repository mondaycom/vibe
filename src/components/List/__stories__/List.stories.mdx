...
import { StoryDescription, Link } from "vibe-storybook-components";
...
<Meta
  ...
  component={List}
  subcomponents={{ ListItem, ListItemIcon, ListItemAvatar }}
...
export const listTemplate = args => {
  ...
  return (
    <List {...args}>
      ...
      <ListItem oonClick={onClick}>Team Power up</ListItem>
  ...
  <Story name="Test">
    {() => {
      ...
      const updateSelectedItem = index => {
        setSelectedItemIndex(prevSelected => (prevSelected === index ? null : index));
      };
      return (
        <List ref={ref} id={"test-list"}>
          {[0, 1, 2, 3, 4, 5].map(index => (
            <ListItem selected={selectedItemIndex === index} onClick={() => updateSelectedItem(index)}>
              {`List item ${index}`}
            </ListItem>
          ))}
        </List>
      ...
      negative: {
        ...
        description: (
          <>
            ...
            <Link href="/?path=/docs/navigation-menu--overview" withoutSpacing>
              Menu
            </Link>
            .
          </>
