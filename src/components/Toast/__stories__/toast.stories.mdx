...
import { Link, UsageGuidelines, Tip, createComponentTemplate } from "vibe-storybook-components";
import {
  ALERT_BANNER,
  ATTENTION_BOX,
  TOOLTIP
...
<Canvas>
  <Story name="Toast with loading" parameters={{ chromatic: { pauseAnimationAtEnd: true } }}>
    {() => {
      return (
