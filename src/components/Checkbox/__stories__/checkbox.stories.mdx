...
<Tip title="Am I using the right component?">For settings that are immediately applied, consider using a <Link href="/?path=/docs/components-toggle--overview" size={Link.sizes.SMALL} withoutSpacing>toggle switch</Link> instead.</Tip>
  ...
  rules={[
    {
      ...
      negative: {
        ...
        description: <>Don't use checkboxes when only one item can be selected from a list. Use <Link href="/?path=/docs/components-radio-button--overview" withoutSpacing>radio buttons</Link> instead.</>
      }
    },
    {
      positive: {
