...
<UsageGuidelines guidelines={[
  ...
  <>Keep the content short and actionable. For longer actions, use the<Link href="/?path=/docs/inputs-dropdown--overview">Dropdown</Link> component.</>,
  <>Button group is used to diplay the same content in diffrent view, if you need to present diffrent content, use <Link href="/?path=/docs/navigation-tabs-tabs--overview">Tabs.</Link></>,
]}/>

<Tip title="Check yourself">Button group will always have one button selected. If you need to display addjacent buttons without selected mode, use  the <Link href="/?path=/docs/buttons-button--overview" size={Link.sizes.SMALL} withoutSpacing>Button</Link> component with “Flat” props. </Tip>
  ...
  rules={[
    {
      ...
      negative: {
        ...
        />,
        description:<>Don’t use a button group when switching between content or object pages. Use <Link href="/?path=/docs/navigation-tabs-tabs--overview" withoutSpacing>Tabs</Link> instead.</>
      }
    },
    {
      positive: {
