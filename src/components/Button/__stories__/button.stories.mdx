...
<UsageGuidelines guidelines={
  [<div> Buttons may contain icon <Link href={"ddddd"} size={Link.sizes.MEDIUM}>jjjsaefds fdsdfds d </Link>, on the left or the right side</div>,
  "ddd"]}/>
  ...
  rules={[
    {
      positive: {
        ...
        description:<>Use 1 or 2 words, no longer than 4 words, <Link href={"www"}>ss</Link> with fewer than 20 characters including spaces.</>
      },
      negative: {
