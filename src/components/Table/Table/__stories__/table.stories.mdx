...
import { createComponentTemplate, Tip } from "vibe-storybook-components";
import {
  emailTableData,
  emailColumns,
  TableAvatar,
...
Click on the rows to highlight them.

<Tip emoji="🔨" title="This feature is WIP">
  This functionality is pre-added for selectable rows feature - checkboxes to multiselect rows will appear on the left.
  Use with caution.
</Tip>
...
<Canvas>
  <Story name="Clickable rows">
    {() => {
      ...
      const onRowClick = (rowId, event) => {
        console.log("### onRowClick: ", rowId, event);
        setSelectedRows(prevState => ({ ...prevState, [rowId]: !prevState?.[rowId] }));
      };
      return (
        <Table errorState={<TableErrorState />} emptyState={<TableEmptyState />} columns={emailColumns}>
          <TableHeader>
          ...
          <TableBody>
            {emailTableData.map(rowItem => (
              <TableRow
                key={rowItem.id}
                highlight={selectedRows[rowItem.id]}
                onClick={event => onRowClick(rowItem.id, event)}
              >
                <TableCell>{rowItem.sentOn}</TableCell>
                <TableCell>{rowItem.subject}</TableCell>
                <TableCell>
