...
import {
  ...
  emailColumnsWithHighlightButton
} from "./helpers";
import {
  LABEL,
  ICON_BUTTON,
...
<Canvas>
  <Story name="Highlightable row">
    {() => {
      ...
      const canRowBeHighlighted = rowItem => rowItem.id === "2";
      const onButtonClick = () => {
        setHighlightRow(prevState => !prevState);
      };
      return (
        <Table
          errorState={<TableErrorState />}
          emptyState={<TableEmptyState />}
          columns={emailColumnsWithHighlightButton}
        >
          <TableHeader>
            {emailColumnsWithHighlightButton.map((headerCell, index) => (
              <TableHeaderCell key={index} title={headerCell.title} />
            ))}
          </TableHeader>
          <TableBody>
            {emailTableData.map(rowItem => (
              <TableRow key={rowItem.id} highlighted={highlightRow && canRowBeHighlighted(rowItem)}>
                <TableCell>{rowItem.sentOn}</TableCell>
                <TableCell>{rowItem.subject}</TableCell>
                <TableCell>
                  <TableAvatar text={rowItem.sentBy} />
                ...
                <TableCell>
                  <Button
                    ...
                    disabled={!canRowBeHighlighted(rowItem)}
                  >
                    Highlight
                  </Button>
                </TableCell>
