...
import {
  ...
  highlightableRowColumns,
  highlightableTableData
} from "./helpers";
import {
  LABEL,
  ICON_BUTTON,
...
<Canvas>
  <Story name="Highlightable row">
    {() => {
      const NO_HIGHLIGHT_STATE = { "1": false, "2": false, "3": false };
      const [highlightRow, setHighlightRow] = useState({ "1": false, "2": true, "3": false });
      const onButtonClick = rowItem => {
        setHighlightRow(prevState => {
          const newState = { ...NO_HIGHLIGHT_STATE };
          newState[rowItem.id] = !prevState[rowItem.id];
          return newState;
        });
      };
      return (
        <Table errorState={<TableErrorState />} emptyState={<TableEmptyState />} columns={highlightableRowColumns}>
          <TableHeader>
            {highlightableRowColumns.map((headerCell, index) => (
              <TableHeaderCell key={index} title={headerCell.title} />
            ))}
          </TableHeader>
          <TableBody>
            {highlightableTableData.map(rowItem => (
              <TableRow key={rowItem.id} highlighted={highlightRow[rowItem.id]}>
                <TableCell>{rowItem.sentOn}</TableCell>
                <TableCell>{rowItem.subject}</TableCell>
                <TableCell>{rowItem.emailsSent}</TableCell>
                <TableCell>
                  <Button kind={Button.kinds.SECONDARY} size={Button.sizes.XS} onClick={() => onButtonClick(rowItem)}>
                    Highlight
                  </Button>
                </TableCell>
              </TableRow>
