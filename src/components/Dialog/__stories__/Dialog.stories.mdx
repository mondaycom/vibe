...
import { CloseSmall, Info } from "../../Icon/Icons";
import {
  MENU,
...
<Canvas>
  <Story name="Close button inside Dialog">
    {() => {
      ...
      return (
        <Flex className="monday-storybook-dialog--story-padding" gap={Flex.gaps.MEDIUM}>
          <Dialog
            ...
            showTrigger={[]}
            hideTrigger={[]}
            // Using callback instead of hideTrigger here cause Controlled and Uncontrolled Dialog states shouldn't be mixed -
            // - putting Dialog.hideShowTriggers.CLICK_OUTSIDE in hideTrigger array won't work
            onClickOutside={e => {
              // Fix to avoid double Dialog reopening when dialog button is clicked (and it's an outside click same time)
            ...
            content={
              <DialogContentContainer>
                <div style={{ width: 300, height: 300 }}>
                  <IconButton icon={CloseSmall} onClick={() => setIsOpen(false)} />
                </div>
              </DialogContentContainer>
            }
          >
