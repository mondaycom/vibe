...
import { useSwitch } from "../../Switch/hooks/useSwitch";
...
<Canvas>
  <Story name="Positions">
    {() => {
      ...
      const { isChecked: checkedTopStart, onChange: onChangeTopStart } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedTopEnd, onChange: onChangeTopEnd } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedBottom, onChange: onChangeBottom } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedBottomStart, onChange: onChangeBottomStart } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedBottomEnd, onChange: onChangeBottomEnd } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedRight, onChange: onChangeRight } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedRightStart, onChange: onChangeRightStart } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedRightEnd, onChange: onChangeRightEnd } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedLeft, onChange: onChangeLeft } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedLeftStart, onChange: onChangeLeftStart } = useSwitch({ defaultChecked: false });
      const { isChecked: checkedLeftEnd, onChange: onChangeLeftEnd } = useSwitch({ defaultChecked: false });
      const modifiers =
        // for prevent dialog to move while scrolling
        [
          {
      ...
      return (
        <Flex className="monday-storybook-dialog--story-padding" gap={Flex.gaps.MEDIUM}>
          ...
          <Dialog
            modifiers={modifiers}
            position={Dialog.positions.BOTTOM_START} // set dialog position
            showTrigger={[]}
            hideTrigger={[]}
            open={checkedBottomStart}
            content={
              <DialogContentContainer>
                <ExampleContent />
              </DialogContentContainer>
          ...
          >
            <Button kind={Button.kinds.SECONDARY} onClick={onChangeBottomStart} active={checkedBottomStart}>
              Bottom start
            </Button>
          </Dialog>
          <Dialog
            modifiers={modifiers}
            position={Dialog.positions.BOTTOM_END} // set dialog position
            showTrigger={[]}
            hideTrigger={[]}
            open={checkedBottomEnd}
            content={
              <DialogContentContainer>
                <ExampleContent />
              </DialogContentContainer>
          ...
          >
            <Button kind={Button.kinds.SECONDARY} onClick={onChangeBottomEnd} active={checkedBottomEnd}>
              Bottom end
            </Button>
          </Dialog>
          <Dialog
...
<Canvas>
  <Story name="Show triggers">
    {() => {
      // For maintain active state of each button according to the dialog open state (this hooks is available for your usage)
      const { checked: clickButtonActive, onChange: onClickClickButton } = useSwitch({ defaultChecked: false });
      const { checked: hoverButtonActive, onChange: onHoverButton } = useSwitch({ defaultChecked: false });
      const { checked: focusButtonActive, onChange: onFocusButton } = useSwitch({ defaultChecked: false });
      const modifiers =
        // for prevent dialog to move while scrolling
        [
          {
