...
export const dialogTemplate = dialogProps => {
  const modifiers =
...
<Tip>
  ...
  <Link href="/?path=/docs/inputs-dropdown--overview">DialogContentContainer</Link> component.
</Tip>
...
<UsageGuidelines
  guidelines={[
    "Dialog can appear from top, bottom, left or right to an element",
    ...
    <>
      ...
      <Link>Menu</Link> and <Link>MenuButton</Link> components.
    </>
  ]}
/>
...
We can chose what will be the related element's trigger which will be responsible for the dialog appearance
...
### Control dialog show / hide states without triggers

<b> Coming soon </b>

### Dialog inside scrollable container

<b> Coming soon </b>

### Dialog for implementing customize menus
