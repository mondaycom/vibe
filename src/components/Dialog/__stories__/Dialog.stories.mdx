...
<UsageGuidelines
  guidelines={[
    ...
    <>
      ...
      <Link href="/?path=/docs/navigation-menu-menu--overview" withoutSpacing>
        Menu
      </Link>{" "}
      and{" "}
      <Link href="?path=/docs/buttons-menubutton--overview" withoutSpacing>
        MenuButton
      </Link>{" "}
      components.
    </>
...
<AttentionBox title="Trying to implement your own customize menu?">
  <>
    ...
    as our <AttentionBoxLink href="/?path=/docs/navigation-menu-menu--overview" text="Menu component" />, because
    creating an over-complicated UI can hurt user experience.
  </>
</AttentionBox>
...
<Canvas>
  <Story name="Show triggers" parameters={{ chromatic: { pauseAnimationAtEnd: true } }}>
    {() => {
      // For maintain active state of each button according to the dialog open state (this hooks is available for your usage)
...
<Canvas>
  <Story name="Dialog with tooltip">
    {() => {
      const modifiers =
        // for prevent dialog to move while scrolling
        [
...
<Tip title="Dev tip">
  ...
  inside a Dialog container and need help displaying the Dropdown's popovers correctly, read more about our Dropdown in popovers
  technical pattern <Link
